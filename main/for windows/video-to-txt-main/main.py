...
MatlabPathDefault=''#no matlab
VideoPathDefault=ProgramPath[0:ProgramPath.rfind('\\')+1]+'test_file\\test.avi'
...
def CheckInput(MatlabPath,VideoPath,DataPath):
    ...
    if (not os.path.isfile(MatlabPath)):
        ...
        time.sleep(Waitingtime)
        return 0
    elif (not MatlabPath[MatlabPath.rfind('\\'):len(MatlabPath)]=='matlab.exe'):
        tkMessageBox.showwarning("Matlab Path Warning","Please choose a true matlab path.")
        MainText.insert("1.0","\nPlease choose a true matlab path.\n")
        return 0
...
quit;"""%(VideoPath,DataPath))
    ...
    time.sleep(Waitingtime)
    MatlabCommand='%s -wait -automation -r "run(\'%s\')"'%(MatlabPath,MainMatlabFilePath)
...
def MainPlayFunction(DataPath):
    ...
    line=fp.readline()
    fp.close()
    MainText.config(height=len(line)*1.1,width=len(line))
    while M<MaxM:
        M=3*N+1
        N=N+1
...
def MatlabPathChoose():
    MatlabPathChoosing=tkFileDialog.askopenfilename(title="Choose Matlab Program",initialdir="C:\\",filetypes=[("Application","*.exe")])
    if MatlabPathChoosing:
        MatlabPath.delete("1.0","end")
        MatlabPath.insert("end",MatlabPathChoosing)
    else:
...
def VideoPathChoose():
    VideoPathChoosing=tkFileDialog.askopenfilename(title="Choose Video File",initialfile=VideoPathDefault,filetypes=[("AVI","*.avi"),("MPEG-1","*.mpg"),("WMV","*.wmv"),("ASF","*.asf"),("ASX","*.asx")])
...
root.iconbitmap(ProgramPath+'\\icon\\icon.ico')

MainFont=tkFont.Font(family="Courier",size="10",weight="normal")
